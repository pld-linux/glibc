...
memoria, igual que para facilitar actualizaciones, cÛdigo com˙n del
...
Sin Èstas, un sistema Linux no podr· funcionar. TambiÈn est· incluido
...
et de faciliter les mises ‡ jour, le code commun au systËme est mis ‡
un endroit et partagÈ entre les programmes. Ce paquetage contient les
bibliothËques partagÈes les plus importantes, la bibliothËque standard
du C et la bibliothËque mathÈmatique standard. Sans celles-ci, un
...
programy w Twoim systemie. Uøywanie przez programy bibliotek z tego
pakietu oszczÍdza miejsce na dysku i pamiÍÊ. WiÍkszo∂Ê kodu
systemowego jest usytuowane w jednym miejscu i dzielone miÍdzy wieloma
programami. Pakiet ten zawiera bardzo waøny zbiÛr bibliotek
standardowych, wspÛ≥dzielonych (dynamicznych) bibliotek C i
matematycznych. Bez glibc system Linux nie jest w stanie funkcjonowaÊ.
...
i˛lemlerini kolayla˛t˝rmak iÁin ortak sistem kodlar˝ tek bir yerde
tutulup programlar aras˝nda payla˛t˝r˝l˝r. Bu paket en ˆnemli ortak
...
NarzÍdzia i dane uøywane przez glibc.

%package devel
Summary:	Additional libraries required to compile
Summary(de):	Weitere Libraries zum Kompilieren
Summary(es):	Bibliotecas adicionales necesarias para la compilaciÛn
...
verwenden (also fast alle), benˆtigt das System diese Standard-Header-
...
nag≥Ûwkowe oraz pliki obiektowe, niezbÍdne do kompilacji programÛw
...
Summary(pl):	Pliki nag≥Ûwkowe do tworzenia programÛw przy uøyciu standardowych bibliotek C
...
Pakiet glibc-headers zawiera pliki nag≥Ûwkowe niezbÍdne do rozwijania
programÛw uøywaj±cych standardowych bibliotek C (uøywanych przez
prawie wszystkie programy). Je∂li tworzymy programy korzystaj±ce ze
standardowych bibliotek C, system wymaga dostÍpno∂ci tych
standardowych plikÛw nag≥Ûwkowych do tworzenia programÛw
wykonywalnych.

Ten pakiet naleøy zainstalowaÊ je∂li zamierzamy tworzyÊ programy
korzystaj±ce ze standardowych bibliotek C.

%package devel-utils
Summary:	Utilities needed for development using standard C libraries
Summary(pl):	NarzÍdzia do tworzenia programÛw przy uøyciu standardowych bibliotek C
Group:		Development/Libraries
...
Pakiet glibc-devel-utils zawiera narzÍdzia niezbÍdne do rozwijania
programÛw uøywaj±cych standardowych bibliotek C (uøywanych przez
prawie wszystkie programy). Je∂li tworzymy programy korzystaj±ce ze
standardowych bibliotek C, system wymaga dostÍpno∂ci tych narzÍdzi do
tworzenia programÛw wykonywalnych.
...
Summary(pl):	Dokumentacja do tworzenia programÛw przy uøyciu standardowych bibliotek C
Group:		Documentation
...
rozwijania programÛw uøywaj±cych standardowych bibliotek C (uøywanych
...
Summary(es):	Demonio de cachÈ del servicio de nombres
...
internacionalizaciÛn de GNU libc.
...
soportados por glibc. En glibc 2.3.x Èse es un fichero grande (aprox.
39 MB) -- si prefiere algo m·s pequeÒo, sÛlo con soporte de unos
locales elegidos, considÈrese instalar localedb-src y regenerar la
base de datos usando el escript localedb-gen (una vez que la base de
datos estÈ creada, localedb-src se podr· desinstalar).
...
(oko≥o 39MB); aby mieÊ co∂ mniejszego, z obs≥ug± tylko wybranych
...
Convierte la codificaciÛn de dados ficheros. Necesita este paquete si
quiere convertir un documento entre una codificaciÛn (juego de
caracteres) y otra, o si tiene instalado alg˙n programa que usa el
Generic Character Set Conversion Interface (interfaz genÈrica de
...
Programy profilowane za pomoc± gprof musz± uøywaÊ tych bibliotek
...
m·s pequeÒo de la biblioteca libc compartida est·ndar.

%description pic -l pl
Archiwum PIC biblioteki GNU C zawiera archiwaln± bibliotekÍ (plik ar)
z≥oøon± z pojedynczych obiektÛw wspÛ≥dzielonych. Uøywana jest do
tworzenia biblioteki bÍd±cej mniejszym podzestawem standardowej
biblioteki wspÛ≥dzielonej libc.
...
El antiguo mÛdulo NYS NSS de glibc
...
MÛdulo de tradicionales bases de datos en ficheros para glibc.
...
MÛdulo hesiod NSS de glibc.

%description -n nss_hesiod -l pl
Modu≥ glibc NSS (Name Service Switch) dostÍpu do baz danych.
...
MÛdulo NSS de glibc para acceder las bases de datos NIS(YP).

%description -n nss_nis -l pl
Modu≥ glibc NSS (Name Service Switch) dostÍpu do baz danych NIS(YP).
...
MÛdulo NSS (Name Service Switch) de glibc para acceder las bases de
datos NIS+.
...
for i in aa af am ang ar az bg bn br bs byn cy de_AT en en@boldquot en@quot \
    en_AU en_CA en_US eo es_AR es_MX es_NI et eu fa fo fr_BE fy ga gez gu gv \
    he hi hsb hy ia id is it_CH iu ka kk kl kn ku kw ky leet lg li lo lt lv \
    mi mk ml mn mr ms mt nds ne nl_BE nn nso oc om or pa pt rm ro ru rw sa \
    se sid sl so sq sr sr@Latn sr@ije ss syr sw ta te tg th ti tig tl tlh tt \
    uk ur uz ve vi wa wal xh yi zu ; do
